...
class ProfileGenerator:
    ...
    def _create_document_elements(self, profile_data, template="professional"):
        ...
        try:
            ...
            # Ausbildung - nur hinzufügen, wenn tatsächlich Ausbildungen vorhanden sind
            ausbildungen = profile_data.get('ausbildung', [])
            if ausbildungen:
                # Überschrift und Inhalte in KeepTogether verpacken, damit die Überschrift nicht allein am Ende einer Seite steht
                ausbildung_elements = []
                ...
                for ausbildung in ausbildungen:
                    try:
                        ...
                        # Wir verpacken die Tabelle und den Spacer in KeepTogether, damit sie nicht über eine Seite verteilt werden
                        ausbildung_elements.append(table)
                        ausbildung_elements.append(Spacer(1, 0.3*cm))
                    except Exception as e:
                        print(f"Fehler bei der Verarbeitung einer Ausbildung: {str(e)}")
                        # Einfache Darstellung als Fallback
                        ausbildung_elements.append(Paragraph(f"{zeitraum} - {institution}", self.custom_styles['Normal']))
                        ausbildung_elements.append(Spacer(1, 0.3*cm))
                
                # Füge alle Ausbildungselemente zusammen als eine untrennbare Einheit hinzu
                elements.append(KeepTogether(ausbildung_elements))
            
            # Fort- und Weiterbildungen - nur hinzufügen, wenn tatsächlich Weiterbildungen vorhanden sind
            ...
            if weiterbildungen:
                ...
                for weiterbildung in weiterbildungen:
                    try:
                        ...
                        # Tabelle und Spacer zur Sammlung hinzufügen
                        weiterbildung_elements.append(table)
                        weiterbildung_elements.append(Spacer(1, 0.3*cm))
                    except Exception as e:
                        print(f"Fehler bei der Verarbeitung einer Weiterbildung: {str(e)}")
                        # Einfache Darstellung als Fallback
                        weiterbildung_elements.append(Paragraph(f"{zeitraum} - {bezeichnung}", self.custom_styles['Normal']))
                        weiterbildung_elements.append(Spacer(1, 0.3*cm))
                
                # Füge alle Weiterbildungselemente zusammen als eine untrennbare Einheit hinzu
